...
// node_modules/.deno/@rdfjs+wrapper@0.34.0/node_modules/@rdfjs/wrapper/dist/DatasetWrapper.js
var DatasetWrapper = class {
  dataset;
  factory;
  //#region DatasetCore
  constructor(dataset, factory2) {
  ...
  *[Symbol.iterator]() {
  ...
  instancesOf(klass, constructor) {
    return this.matchSubjectsOf(constructor, this.factory.namedNode(RDF2.type), this.factory.namedNode(klass));
  }
  named(graph, klass) {
...
// node_modules/.deno/@solid+object@0.6.0/node_modules/@solid/object/dist/webid/WebIdDataset.js
var WebIdDataset = class extends DatasetWrapper {
  get mainSubject() {
...
function termFromId(id, factory2, nested) {
  factory2 = factory2 || DataFactory2;
  if (!id) return factory2.defaultGraph();
  switch (id[0]) {
    case "?":
    ...
    case '"':
      if (factory2 === DataFactory2) return new Literal2(id);
      if (id[id.length - 1] === '"') return factory2.literal(id.substr(1, id.length - 2));
...
function termToId2(term, nested) {
  ...
  if (term instanceof Term2 && term.termType !== "Quad") return term.id;
  if (!term) return DEFAULTGRAPH2.id;
  switch (term.termType) {
    case "NamedNode":
      return term.value;
    case "BlankNode":
    ...
    case "DefaultGraph":
      return "";
    case "Literal":
      return `"${term.value}"${term.language ? `@${term.language}` : term.datatype && term.datatype.value !== xsd3.string ? `^^${term.datatype.value}` : ""}`;
    case "Quad":
      const res = [
        termToId2(term.subject, true),
        termToId2(term.predicate, true),
        termToId2(term.object, true)
      ];
...
// node_modules/.deno/n3@1.26.0/node_modules/n3/src/N3Util.js
function isDefaultGraph(term) {
  return !!term && term.termType === "DefaultGraph";
}

// node_modules/.deno/n3@1.26.0/node_modules/n3/src/Util.js
function escapeRegex(regex) {
  return regex.replace(/[\]\/\(\)\*\+\?\.\\\$]/g, "\\$&");
}

// node_modules/.deno/n3@1.26.0/node_modules/n3/src/BaseIRI.js
...
// node_modules/.deno/n3@1.26.0/node_modules/n3/src/N3Writer.js
var DEFAULTGRAPH3 = N3DataFactory_default2.defaultGraph();
var { rdf: rdf3, xsd: xsd4 } = IRIs_default2;
var escape = /["\\\t\n\r\b\f\u0000-\u0019\ud800-\udbff]/;
var escapeAll = /["\\\t\n\r\b\f\u0000-\u0019]|[\ud800-\udbff][\udc00-\udfff]/g;
...
var SerializedTerm = class extends Term2 {
...
var N3Writer = class {
  ...
  get _inDefaultGraph() {
    return DEFAULTGRAPH3.equals(this._graph);
  }
  ...
  _writeQuad(subject, predicate, object, graph, done) {
    try {
      if (!graph.equals(this._graph)) {
        this._write((this._subject === null ? "" : this._inDefaultGraph ? ".\n" : "\n}\n") + (DEFAULTGRAPH3.equals(graph) ? "" : `${this._encodeIriOrBlank(graph)} {
`));
        this._graph = graph;
        this._subject = null;
      }
...
` : " .\n"}`;
  ...
  quadsToString(quads) {
    let quadsString = "";
    for (const quad3 of quads) quadsString += this.quadToString(quad3.subject, quad3.predicate, quad3.object, quad3.graph);
    return quadsString;
  }
  ...
  _encodeLiteral(literal3) {
    ...
    if (this._lineMode) {
      if (literal3.datatype.value === xsd4.string) return `"${value}"`;
    } else {
  ...
  addQuad(subject, predicate, object, graph, done) {
    ...
    else if (typeof graph === "function") this._writeQuad(subject, predicate, object, DEFAULTGRAPH3, graph);
    else this._writeQuad(subject, predicate, object, graph || DEFAULTGRAPH3, done);
  }
...
// node_modules/.deno/n3@1.26.0/node_modules/n3/src/N3Store.js
...
var N3EntityIndex = class {
  constructor(options = {}) {
    ...
    this._factory = options.factory || N3DataFactory_default2;
  }
  _termFromId(id) {
    if (id[0] === ".") {
      const entities = this._entities;
  ...
  _termToNumericId(term) {
    ...
    return this._ids[termToId2(term)];
  }
  _termToNewNumericId(term) {
    const str = term && term.termType === "Quad" ? `.${this._termToNewNumericId(term.subject)}.${this._termToNewNumericId(term.predicate)}.${this._termToNewNumericId(term.object)}${isDefaultGraph(term.graph) ? "" : `.${this._termToNewNumericId(term.graph)}`}` : termToId2(term);
...
var N3Store = class _N3Store {
  ...
  *_findInIndex(index0, key0, key1, key2, name0, name1, name2, graphId) {
  ...
  removeMatches(subject, predicate, object, graph) {
    const stream = new import_readable_stream.Readable({
      objectMode: true
    });
  ...
  *readQuads(subject, predicate, object, graph) {
  ...
  some(callback, subject, predicate, object, graph) {
    for (const quad3 of this.readQuads(subject, predicate, object, graph)) if (callback(quad3, this)) return true;
    return false;
  }
  ...
  extractLists({ remove = false, ignoreErrors = false } = {}) {
    ...
    const tails = this.getQuads(null, IRIs_default2.rdf.rest, IRIs_default2.rdf.nil, null);
    ...
    tails.forEach((tailQuad) => {
      ...
      while (current && !malformed) {
        ...
        for (let i = 0; i < subjectQuads.length && !malformed; i++) {
          quad3 = subjectQuads[i];
          if (!quad3.graph.equals(graph)) malformed = onError(current, "not confined to single graph");
          else if (head) malformed = onError(current, "has non-list arcs out");
          else if (quad3.predicate.value === IRIs_default2.rdf.first) {
            if (first) malformed = onError(current, "has multiple rdf:first arcs");
            else toRemove.push(first = quad3);
          } else if (quad3.predicate.value === IRIs_default2.rdf.rest) {
            if (rest) malformed = onError(current, "has multiple rdf:rest arcs");
            else toRemove.push(rest = quad3);
  ...
  addAll(quads) {
    ...
    if (Array.isArray(quads)) this.addQuads(quads);
      ...
      for (const quad3 of quads) this.add(quad3);
    }
    return this;
  }
  /**
  ...
  contains(other) {
    ...
    if (!(other instanceof _N3Store) || this._entityIndex !== other._entityIndex) return other.every((quad3) => this.has(quad3));
  ...
  deleteMatches(subject, predicate, object, graph) {
    for (const quad3 of this.match(subject, predicate, object, graph)) this.removeQuad(quad3);
    return this;
  }
  /**
  ...
  filter(iteratee) {
    ...
    for (const quad3 of this) if (iteratee(quad3, this)) store.add(quad3);
    return store;
  }
  /**
  ...
  map(iteratee) {
    ...
    for (const quad3 of this) store.add(iteratee(quad3, this));
    return store;
  }
  /**
  ...
  reduce(callback, initialValue) {
    ...
    for (const quad3 of iter) accumulator = callback(accumulator, quad3, this);
    return accumulator;
  }
  /**
      ...
      for (const graphKey in graphs) {
        ...
        if (content = graphs[graphKey]) {
          if (!subjectId && predicateId) {
            if (predicates = indexMatch(content.predicates, [
              predicateId,
              objectId,
              subjectId
            ])) {
              subjects = indexMatch(content.subjects, [
                subjectId,
                predicateId,
                objectId
              ]);
              objects = indexMatch(content.objects, [
                objectId,
                subjectId,
                predicateId
              ]);
            ...
            if (objects = indexMatch(content.objects, [
              objectId,
              subjectId,
              predicateId
            ])) {
              ...
              predicates = indexMatch(content.predicates, [
                predicateId,
                objectId,
                subjectId
              ]);
            }
          } else if (subjects = indexMatch(content.subjects, [
            subjectId,
            predicateId,
            objectId
          ])) {
...
(function(LiteralFrom3) {
  function anyUriString(value, factory2) {
    return factory2.literal(value, factory2.namedNode(XSD4.anyUri));
  }
  LiteralFrom3.anyUriString = anyUriString;
  function anyUriUrl(value, factory2) {
    return anyUriString(value.toString(), factory2);
  ...
  LiteralFrom3.anyUriUrl = anyUriUrl;
  function base64(value, factory2) {
    return factory2.literal(value.toBase64(), factory2.namedNode(XSD4.base64Binary));
  }
  LiteralFrom3.base64 = base64;
  function boolean(value, factory2) {
    return factory2.literal(value.toString(), factory2.namedNode(XSD4.boolean));
  }
  LiteralFrom3.boolean = boolean;
  function date(value, factory2) {
    return factory2.literal(value.toISOString(), factory2.namedNode(XSD4.date));
  }
  LiteralFrom3.date = date;
  function dateTime(value, factory2) {
    return factory2.literal(value.toISOString(), factory2.namedNode(XSD4.dateTime));
  }
  LiteralFrom3.dateTime = dateTime;
  function double(value, factory2) {
    return factory2.literal(value.toString(), factory2.namedNode(XSD4.double));
  }
  LiteralFrom3.double = double;
  function hex(value, factory2) {
    return factory2.literal(value.toHex(), factory2.namedNode(XSD4.hexBinary));
  }
  LiteralFrom3.hex = hex;
  function langString(value, factory2) {
    return factory2.literal(value.string, {
...
var WrappingMap2 = class {
  ...
  *entries() {
    for (const quad3 of this.matches) {
      yield this.termAs(new TermWrapper2(quad3.object, this.subject.dataset, this.subject.factory));
    }
  }
  *keys() {
    for (const [key] of this) {
...
var AnyTermWithContext = class extends AnyTerm {
  ...
  factory;
  constructor(term, dataset, factory2) {
    super(typeof term === "string" ? factory2.namedNode(term) : term);
    this.dataset = dataset;
    this.factory = factory2;
  }
};
...
var TermWrapper2 = class _TermWrapper extends AnyTermWithContext {
  get [Symbol.toStringTag]() {
    return this.constructor.name;
  }
  singular(p, termAs) {
  ...
  objects(p, termAs, termFrom) {
    return new WrappingSet2(this, p, termAs, termFrom);
  }
  map(p, termAs, termFrom) {
    return new WrappingMap2(this, p, termAs, termFrom);
  }
...
var ListItem2 = class _ListItem extends TermWrapper2 {
  ...
  get firstRaw() {
    return this.singularNullable(RDF5.first, TermAs2.term);
  }
  set firstRaw(value) {
    this.overwriteNullable(RDF5.first, value, TermFrom2.itself);
  }
  get restRaw() {
    return this.singularNullable(RDF5.rest, TermAs2.term);
  }
  set restRaw(value) {
    this.overwriteNullable(RDF5.rest, value, TermFrom2.itself);
  }
  get isListItem() {
    return this.firstRaw !== void 0 && this.restRaw !== void 0;
  }
  get isNil() {
    return this.equals(this.factory.namedNode(RDF5.nil));
  }
  get first() {
    return this.singular(RDF5.first, this.termAs);
  }
  set first(value) {
    this.overwrite(RDF5.first, value, this.termFrom);
  }
  get rest() {
    return this.singular(RDF5.rest, (w) => new _ListItem(w, w.dataset, w.factory, this.termAs, this.termFrom));
  }
  set rest(value) {
    this.overwrite(RDF5.rest, value, TermFrom2.instance);
  }
  pop() {
    try {
      return this.first;
      ...
      this.firstRaw = void 0;
      this.restRaw = this.factory.namedNode(RDF5.nil);
    }
  }
  *items() {
    if (this.firstRaw === void 0) {
...
var Overwriter2 = class extends TermWrapper2 {
  ...
  set listNode(object) {
    this.overwrite(this.p, object, TermFrom2.instance);
  }
};
...
var RdfList2 = class {
  ...
  push(...items) {
    const nil = this.subject.factory.namedNode(RDF5.nil);
    for (const item of items) {
...
function ensureType(term) {
  if (!(term instanceof TermWrapper2)) {
...
(function(LiteralAs3) {
  ...
  function boolean(term) {
    ...
    ensureType2(term);
    ensureLiteral(term);
    ensureDatatype2(term, XSD4.boolean);
    return term.value === "true" || term.value === "1";
  }
  LiteralAs3.boolean = boolean;
  function date(term) {
  ...
  function url(term) {
    ...
    ensureType2(term);
    ensureLiteral(term);
    ensureDatatype2(term, XSD4.anyUri);
    return new URL(term.value);
  }
  LiteralAs3.url = url;
...
var integerDatatypes2 = [
  XSD4.integer,
  XSD4.nonPositiveInteger,
  XSD4.long,
  XSD4.nonNegativeInteger,
  XSD4.negativeInteger,
  XSD4.int,
  XSD4.unsignedLong,
  XSD4.positiveInteger,
  XSD4.short,
  XSD4.unsignedInt,
  XSD4.byte,
  XSD4.unsignedShort,
...
function ensureLiteral(term) {
  if (term.termType !== "Literal") {
    throw new TermTypeError2(term, "Literal");
  }
}
...
function ensureNamedNode(term) {
  if (term.termType !== "NamedNode") {
    throw new TermTypeError2(term, "NamedNode");
  }
}
...
// node_modules/.deno/@rdfjs+wrapper@0.32.0/node_modules/@rdfjs/wrapper/dist/DatasetWrapper.js
var DatasetCoreBase = class {
  dataset;
  factory;
  constructor(dataset, factory2) {
    this.dataset = dataset;
...
var DatasetWrapper2 = class extends DatasetCoreBase {
  *subjectsOf(predicate, termWrapper) {
  ...
  *objectsOf(predicate, termWrapper) {
    for (const q of this.matchObjectsOf(termWrapper, void 0, this.factory.namedNode(predicate))) {
      yield q;
    }
  }
  *instancesOf(predicate, constructor) {
    for (const q of this.matchSubjectsOf(constructor, this.factory.namedNode(RDF5.type), this.factory.namedNode(predicate))) {
      yield q;
    }
  }
  *matchSubjectsOf(termWrapper, predicate, object, graph) {
...
// data/rdf.ts
var ScheduleRdf = class extends TermWrapper2 {
  get start() {
    return this.singular(ICAL.dtstart, LiteralAs2.string);
  }
  set start(value) {
    this.overwrite(ICAL.dtstart, value, LiteralFrom2.string);
  }
  get end() {
    return this.singularNullable(ICAL.dtend, LiteralAs2.string);
  }
  set end(value) {
    this.overwriteNullable(ICAL.dtend, value, LiteralFrom2.string);
  }
};
var LocationRdf = class extends TermWrapper2 {
  get name() {
    return this.singular(FOAF2.name, LiteralAs2.string);
  }
  set name(value) {
    this.overwrite(FOAF2.name, value, LiteralFrom2.string);
  }
  get lat() {
    return this.singular(WGS84.lat, LiteralAs2.number);
  }
  set lat(value) {
    this.overwrite(WGS84.lat, value, LiteralFrom2.double);
  }
  get lng() {
    return this.singular(WGS84.long, LiteralAs2.number);
  }
  set lng(value) {
    this.overwrite(WGS84.long, value, LiteralFrom2.double);
  }
  get details() {
    return this.singular(COMIDAS.details, LiteralAs2.string);
  }
  set details(value) {
    this.overwrite(COMIDAS.details, value, LiteralFrom2.string);
  }
};
var AvailabilityRdf = class extends TermWrapper2 {
  get schedule() {
    return this.singular(COMIDAS.schedule, TermAs2.instance(ScheduleRdf));
  }
  set schedule(value) {
    this.overwrite(COMIDAS.schedule, value, TermFrom2.instance);
  }
  get location() {
    return this.singular(ICAL.location, TermAs2.instance(LocationRdf));
  }
  set location(value) {
    this.overwrite(ICAL.location, value, TermFrom2.instance);
  }
  get provisions() {
    return this.objects(COMIDAS.provision, LiteralAs2.string, LiteralFrom2.string);
  }
  get role() {
    return this.singular(COMIDAS.role, LiteralAs2.string);
  }
  set role(value) {
    this.overwrite(COMIDAS.role, value, LiteralFrom2.string);
  }
  get details() {
    return this.singularNullable(ICAL.summary, LiteralAs2.string);
  }
  set details(value) {
...
var ComidasAgent = class extends TermWrapper2 {
  ...
  get locations() {
    return this.objects(COMIDAS.hasLocation, TermAs2.instance(LocationRdf), TermFrom2.instance);
  }
  get availabilities() {
    return this.objects(COMIDAS.availability, TermAs2.instance(AvailabilityRdf), TermFrom2.instance);
  }
  get flags() {
    return this.objects(COMIDAS.flag, LiteralAs2.string, LiteralFrom2.string);
  }
};
var ComidasDataset = class extends DatasetWrapper2 {
  allAgents() {
    return this.instancesOf(COMIDAS.Agent, ComidasAgent);
  }
};
function scheduleToRdf(schedule, dataset, factory2) {
  const node = new ScheduleRdf(factory2.blankNode(), dataset, factory2);
  node.start = schedule.start;
  if (schedule.end !== void 0) node.end = schedule.end;
  return node;
}
function locationToRdf(location, dataset, factory2) {
  const node = new LocationRdf(factory2.blankNode(), dataset, factory2);
...
function availabilityToRdf(av, dataset, factory2) {
  const node = new AvailabilityRdf(factory2.blankNode(), dataset, factory2);
  node.schedule = scheduleToRdf(av.schedule, dataset, factory2);
  node.location = locationToRdf(av.location, dataset, factory2);
  node.role = av.role;
  if (av.details !== void 0) node.details = av.details;
  for (const p of av.provisions) node.provisions.add(p);
  return node;
}
function agentToRdf(agent, dataset, factory2) {
  const node = new ComidasAgent(agent["@id"], dataset, factory2);
  dataset.add(factory2.quad(factory2.namedNode(agent["@id"]), factory2.namedNode(RDF6.type), factory2.namedNode(COMIDAS.Agent)));
  node.name = agent.name;
  for (const loc of agent.locations) {
    node.locations.add(locationToRdf(loc, dataset, factory2));
  }
  for (const av of agent.availabilities) {
    node.availabilities.add(availabilityToRdf(av, dataset, factory2));
  }
  if (agent.flags) {
...
function scheduleFromRdf(node) {
  const s = {
    start: node.start
  };
  if (node.end !== void 0) s.end = node.end;
  return s;
}
function locationFromRdf(node) {
  return {
    name: node.name,
    coordinates: {
      lat: node.lat,
...
function availabilityFromRdf(node) {
  const av = {
  ...
  if (node.details !== void 0) av.details = node.details;
  return av;
}
function agentFromRdf(node) {
  const agent = {
    "@id": node.id,
    name: node.name,
    locations: [
...
async function getStorageUrl(webId) {
  const { store } = await fetchTurtle(webId);
  const profile = new WebIdDataset(store, N3DataFactory_default2);
...
async function readAgentFromPod(webId) {
  ...
  try {
    ...
    const ds = new ComidasDataset(store, N3DataFactory_default2);
...
async function writeAgentToPod(agent, webId) {
  const storage = await getStorageUrl(webId);
  ...
  if (resp.status === 412) {
    throw new Error("Conflict: the resource was modified by someone else. Reload and try again.");
  }
  if (!resp.ok) {
