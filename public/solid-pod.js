...
var LiteralDatatypeError = class extends TermError {
  ...
  /**
     ...
     * @param cause - The specific original cause of the error.
     */
  constructor(literal2, datatypes, cause) {
    super(literal2, `Datatype must be one of ${[
      ...datatypes
    ].join()} but was ${literal2.datatype}`, cause);
...
var WrappingMap = class {
  ...
  *entries() {
    for (const quad2 of this.matches) {
      yield this.termAs(new TermWrapper(quad2.object, this.subject.dataset, this.subject.factory));
    }
  }
  *keys() {
...
// node_modules/.deno/n3@1.26.0/node_modules/n3/src/N3Lexer.js
...
// node_modules/.deno/n3@1.26.0/node_modules/n3/src/N3Parser.js
...
` : " .\n"}`;
  ...
  _encodeLiteral(literal2) {
    ...
    if (literal2.language) return `"${value}"@${literal2.language}`;
    if (this._lineMode) {
      if (literal2.datatype.value === xsd3.string) return `"${value}"`;
    } else {
  ...
  addQuad(subject, predicate, object, graph, done) {
    ...
    else if (typeof graph === "function") this._writeQuad(subject, predicate, object, DEFAULTGRAPH2, graph);
    else this._writeQuad(subject, predicate, object, graph || DEFAULTGRAPH2, done);
  }
...
(function(LiteralFrom3) {
  function anyUriString(value, factory) {
    return factory.literal(value, factory.namedNode(XSD3.anyUri));
  }
  LiteralFrom3.anyUriString = anyUriString;
  function anyUriUrl(value, factory) {
    return anyUriString(value.toString(), factory);
  ...
  LiteralFrom3.anyUriUrl = anyUriUrl;
  function base64(value, factory) {
    return factory.literal(value.toBase64(), factory.namedNode(XSD3.base64Binary));
  }
  LiteralFrom3.base64 = base64;
  function boolean(value, factory) {
    return factory.literal(value.toString(), factory.namedNode(XSD3.boolean));
  }
  LiteralFrom3.boolean = boolean;
  function date(value, factory) {
    return factory.literal(value.toISOString(), factory.namedNode(XSD3.date));
  }
  LiteralFrom3.date = date;
  function dateTime(value, factory) {
    return factory.literal(value.toISOString(), factory.namedNode(XSD3.dateTime));
  }
  LiteralFrom3.dateTime = dateTime;
  function double(value, factory) {
    return factory.literal(value.toString(), factory.namedNode(XSD3.double));
  }
  LiteralFrom3.double = double;
  function hex(value, factory) {
    return factory.literal(value.toHex(), factory.namedNode(XSD3.hexBinary));
  }
  LiteralFrom3.hex = hex;
  function langString(value, factory) {
    return factory.literal(value.string, {
...
var WrappingMap2 = class {
  ...
  *entries() {
    for (const quad2 of this.matches) {
      yield this.termAs(new TermWrapper2(quad2.object, this.subject.dataset, this.subject.factory));
    }
  }
  *keys() {
    for (const [key] of this) {
...
var ListItem2 = class _ListItem extends TermWrapper2 {
  ...
  get firstRaw() {
    return this.singularNullable(RDF4.first, TermAs2.term);
  }
  set firstRaw(value) {
    this.overwriteNullable(RDF4.first, value, TermFrom2.itself);
  }
  get restRaw() {
    return this.singularNullable(RDF4.rest, TermAs2.term);
  }
  set restRaw(value) {
    this.overwriteNullable(RDF4.rest, value, TermFrom2.itself);
  }
  get isListItem() {
    return this.firstRaw !== void 0 && this.restRaw !== void 0;
  }
  get isNil() {
    return this.equals(this.factory.namedNode(RDF4.nil));
  }
  get first() {
    return this.singular(RDF4.first, this.termAs);
  }
  set first(value) {
    this.overwrite(RDF4.first, value, this.termFrom);
  }
  get rest() {
    return this.singular(RDF4.rest, (w) => new _ListItem(w, w.dataset, w.factory, this.termAs, this.termFrom));
  }
  set rest(value) {
    this.overwrite(RDF4.rest, value, TermFrom2.instance);
  }
  pop() {
    try {
      return this.first;
      ...
      this.firstRaw = void 0;
      this.restRaw = this.factory.namedNode(RDF4.nil);
    }
  }
  *items() {
    if (this.firstRaw === void 0) {
...
var RdfList2 = class {
  ...
  push(...items) {
    const nil = this.subject.factory.namedNode(RDF4.nil);
    for (const item of items) {
...
(function(LiteralAs3) {
  ...
  function boolean(term) {
    ...
    ensureType2(term);
    ensureLiteral(term);
    ensureDatatype2(term, XSD3.boolean);
    return term.value === "true" || term.value === "1";
  }
  LiteralAs3.boolean = boolean;
  function date(term) {
  ...
  function url(term) {
    ...
    ensureType2(term);
    ensureLiteral(term);
    ensureDatatype2(term, XSD3.anyUri);
    return new URL(term.value);
  }
  LiteralAs3.url = url;
...
var integerDatatypes2 = [
  XSD3.integer,
  XSD3.nonPositiveInteger,
  XSD3.long,
  XSD3.nonNegativeInteger,
  XSD3.negativeInteger,
  XSD3.int,
  XSD3.unsignedLong,
  XSD3.positiveInteger,
  XSD3.short,
  XSD3.unsignedInt,
  XSD3.byte,
  XSD3.unsignedShort,
...
var DatasetWrapper2 = class extends DatasetCoreBase {
  ...
  *instancesOf(predicate, constructor) {
    for (const q of this.matchSubjectsOf(constructor, this.factory.namedNode(RDF4.type), this.factory.namedNode(predicate))) {
      yield q;
    }
  }
  *matchSubjectsOf(termWrapper, predicate, object, graph) {
...
function scheduleToRdf(schedule, dataset, factory) {
...
function locationToRdf(location, dataset, factory) {
...
function availabilityToRdf(av, dataset, factory) {
...
function agentToRdf(agent, dataset, factory) {
  const node = new ComidasAgent(agent["@id"], dataset, factory);
  dataset.add(factory.quad(factory.namedNode(agent["@id"]), factory.namedNode(RDF5.type), factory.namedNode(COMIDAS.Agent)));
...
async function getStorageUrl(webId) {
  ...
  const profile = new WebIdDataset(store, N3DataFactory_default);
...
async function readAgentFromPod(webId) {
  ...
  try {
    ...
    const ds = new ComidasDataset(store, N3DataFactory_default);
...
async function writeAgentToPod(agent, webId) {
  ...
  const store = new N3Store();
  agentToRdf(agent, store, N3DataFactory_default);
  const turtle = await new Promise((resolve, reject) => {
    ...
    for (const quad2 of store) writer.addQuad(quad2);
